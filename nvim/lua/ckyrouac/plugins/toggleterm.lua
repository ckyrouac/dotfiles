return {
  {
	...
	on_open = function(term)
	  vim.cmd("startinsert!")
	  vim.api.nvim_buf_set_keymap(0, "t", "<A-x>", [[<C-\><C-o>:lua _dropdown_toggle()<cr>]], {noremap = true, silent=true})
	  vim.api.nvim_buf_set_keymap(0, "t", "<esc><esc>", [[<C-\><C-o>:lua _dropdown_toggle()<cr>]], {noremap = true, silent=true})
	end,
      })
